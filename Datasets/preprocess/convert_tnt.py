...
def load_COLMAP_poses(cam_file, img_dir, tf='w2c'):
    ...
    if tf == 'c2w':
        ...
        for k, v in poses.items():
            print(f"k {k}, names {len(names)}, {names[-1]}")
...
def init_colmap(args):
    ...
    for scene in scene_list:
        ...
        with open(os.path.join(scene_path, 'scene.json'), 'w') as fp:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tnt_path', type=str, default='/home/wangsc/Documents/SurR/datasets/tnt_dataset/', help='Path to tanks and temples dataset')
